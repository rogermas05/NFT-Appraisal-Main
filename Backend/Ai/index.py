...
def select_random_models(models_data, count=3, excluded_models=None):
    """Select random models from the available models list"""
    if excluded_models is None:
        excluded_models = []
    
    # Extract only free models
    free_models = [
        model for model in models_data["data"] 
        if model["id"].endswith(":free") and model["id"] not in excluded_models
    ]
    ...
    if len(free_models) <= count:
        ...
        selected_models = random.sample(free_models, count)
    
    return [
        {
            "id": model["id"],
            "max_tokens": 3500,  # High token limit to avoid truncation
            "temperature": 0.7
        }
...
async def main():
    ...
    # Try to load free_models.json if it exists
    free_models_path = Path("free_models.json")
    models_data = None
    
    if free_models_path.exists():
        try:
            models_data = load_json(free_models_path)
            print_colored(f"Loaded {len(models_data.get('data', []))} models from free_models.json", "green")
        except Exception as e:
            print_colored(f"Error loading free_models.json: {e}", "red")
    
    # Load or create the consensus configuration
    config_file = config_path / "consensus_config.json"
    
    # If we have models data, use it to select random models
    if models_data and not config_file.exists():
        # Select 3 random models for generating responses
        selected_models = select_random_models(models_data, count=3)
        
        # Use a more powerful model for the aggregator
        # Preferring larger models (more likely to have good synthesis capabilities)
        ...
        # Find the first available aggregator model
        ...
        for candidate in aggregator_candidates:
            for model in models_data["data"]:
                if model["id"] == candidate:
                    aggregator_model = {
                        "id": model["id"],
                        "max_tokens": 3500,  # High token limit for aggregation
        ...
        # Fall back to llama-3.2-3b if no preferred model is found
        ...
        print_colored(f"Created configuration with randomly selected models at {config_file}", "green")
        print_colored("Selected models:", "green")
        for model in selected_models:
            print(f"  - {model['id']}")
        print_colored(f"Aggregator model: {aggregator_model['id']}", "green")
